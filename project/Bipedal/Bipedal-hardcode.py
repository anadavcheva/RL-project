...
HULL_FD = Box2D.b2FixtureDef(
    shape=Box2D.b2PolygonShape(vertices=[(x / SCALE, y / SCALE) for x, y in HULL_POLY]),
    density=5.0,
    friction=0.1,
    categoryBits=0x0020,
...
LEG_FD = Box2D.b2FixtureDef(
    shape=Box2D.b2PolygonShape(box=(LEG_W / 2, LEG_H / 2)),
    density=1.0,
    restitution=0.0,
    categoryBits=0x0020,
    maskBits=0x001,
...
LOWER_FD = Box2D.b2FixtureDef(
    shape=Box2D.b2PolygonShape(box=(0.8 * LEG_W / 2, LEG_H / 2)),
    density=1.0,
    restitution=0.0,
    categoryBits=0x0020,
    maskBits=0x001,
...
class BipedalWalker(gym.Env, EzPickle):
    ...
    def __init__(self):
        ...
        self.fd_polygon = Box2D.b2FixtureDef(
            shape=Box2D.b2PolygonShape(vertices=[(0, 0), (1, 0), (1, -1), (0, -1)]),
            friction=FRICTION,
        )

        self.fd_edge = Box2D.b2FixtureDef(
            shape=Box2D.b2EdgeShape(vertices=[(0, 0), (1, 1)]),
            friction=FRICTION,
            categoryBits=0x0001,
        )
    ...
    def reset(self):
        ...
        self.world.contactListener = self.world.contactListener
    ...
    def render(self, mode="human"):
        ...
        for obj in self.drawlist:
            for f in obj.fixtures:
                ...
                if type(f.shape) is Box2D.b2CircleShape:
